...
def write_minimal_waveforms(path: str | Path, config: dict[str, Any], process_state: dict[str, Any]) -> None:
    ...
    rows = [
        ...
        _waveform_row(
            float(timeline["breakdown_end_s"]),
            "breakdown_end",
            float(stage_1["Ip_at_breakdown_end_MA"]),
            stage_1["coil_state_at_breakdown_end"],
            device,
        ),
        _waveform_row(
            float(timeline["rampup_end_s"]),
            "rampup_end",
            float(stage_2["Ip_at_rampup_end_MA"]),
            stage_2["coil_state_at_rampup_end"],
            device,
        ),
        _waveform_row(
            float(timeline["flattop_end_s"]),
            "flattop_end",
            float(stage_3["Ip_at_flattop_end_MA"]),
            stage_3["coil_state_at_flattop_end"],
            device,
        ),
    ]
...
def main() -> None:
    args = parse_args()
    process_state = run_pipeline(args.config)
